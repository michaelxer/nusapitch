...
from nusapitch import ai, backups, db, email_client, imports, privacy, profiles, replies, research, setup_advisor, setup_validation, suppression  # noqa: E402
...
def dashboard(conn) -> None:
    ...
    use_llm_advisor = st.checkbox("Use configured LLM for setup recommendations")
    st.caption("LLM advisor sends a setup summary to your configured provider. Leave it off for local-only checks.")
    if st.button("Check setup before operation"):
        result = setup_advisor.check_setup_readiness(conn, use_llm=use_llm_advisor)
        if result["ready"]:
            st.success("Setup is ready to start operation.")
        else:
            st.error("Setup is not ready yet.")
        if result["blockers"]:
            st.markdown("**Fix before sending**")
        ...
        for item in result["recommendations"]:
            st.write(f"- {item}")
        if result["ai_used"]:
            st.caption("Recommendations were generated by the configured LLM advisor.")
        elif result["ai_error"]:
            st.warning(f"LLM advisor was not used: {result['ai_error']}")
